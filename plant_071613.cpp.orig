	...
	else if(develop->get_LvsAppeared() <= 1.0)
	{
		...
		for (int i = 1; i <= develop->get_LvsInitiated() ; i++)
		{
			...
			else
			{
				...
				CH2O_pool += seed_reserve*T_effect*(1/30)*develop->get_dt(); // assume it takes 30 days to exhaust seed reserve at Topt
				calcMaintRespiration(weather);
				CH2O_allocation(weather);
				seed_reserve = __max(0.0,seed_reserve-CH2O_pool*(C_MW/CH2O_MW)/C_conc);
			}
		}
		return;
	}
...
//so individual leaf doesn't have mass but the first or the last one has it all
{
	...
	{
		double leaf = 0.0;
		// apply leaf and stalk senescence and degradation. this is mainly to emulate living structural parts
...
//   double tmprEffect = g2/g1;
   ...
   // 0.2 is value for hourly interval, Grant (1989)
	...
	shootPart = Yg*(Fraction*(CH2O_supply-maintRespiration)); // gCH2O partitioned to shoot
	rootPart = Yg*((1-Fraction)*(CH2O_supply-maintRespiration)); // gCH2O partitioned to roots
...
//	   reservePart = 0.0;
   ...
   {
	   leafPart = shootPart*0.80;
	   sheathPart = shootPart*0.1;
	   scapePart = shootPart*0.1;
//	   reservePart = 0.0;
	   tunicPart = 0.0;
	   basalplatePart = 0.0;
	   clovePart = 0.0;
...
//		reservePart = shootPart*__max(0.1*0.05,0); 
		...
		//for now LT reserve also gets a min portion of assimilates, the way partitioning is done here now is rudimentary without empirical data to support from specifically designed experiments
   ...
   else if (!develop->ScapeRemoved())
   {
		...
		sheathPart = shootPart*__max(0.6*0.2,0);
		scapePart = shootPart*__max(0.6*0.6,0);
//		reservePart = shootPart*__max(0.6*0.1,0);

		basalplatePart = shootPart*__max(0.4*0.20,0);
		clovePart = shootPart*__max(0.4*0.7,0);
		tunicPart = shootPart*__max(0.4*0.1,0);
   }
   else if (!develop->Matured())
   {
       ...
       clovePart = shootPart*0.65;
	   basalplatePart = shootPart*0.1;
...
//	   reservePart = shootPart*0.1;
   ...
   double pseudostemPart = sheathPart; // Assumes part of pseudostem serves as long-term CH2O reserve. In reality, this should include clove storage, and degradable portion of other structural parts, 7-15-13 SK
   ...
   this->CH2O_reserve += __max(0.0, CH2O_pool); // any unused CH2O in the short-term pool is translocated to long-term reserve
         CH2O_pool -= CH2O_pool; // deplete the short-term pool
// Flush CH2O_pool at midnight in update routine above instead of every time step
...
void CPlant::writeNote(const TWeather & w)
{
	...
	if (FLOAT_EQ(develop->scapeAppear.daytime, w.daytime)){s = "Scaping";}
